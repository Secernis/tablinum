...
const CASES = [
  ...
  ["raw git switch -c", { tool_input: { command: "git switch -c Feature_Foo" }, tool_name: "Bash" }, true],
  ["raw git checkout -b", { tool_input: { command: "git checkout -b feat/foo" }, tool_name: "Bash" }, true],
  ["raw git branch <name>", { tool_input: { command: "git branch feat/foo" }, tool_name: "Bash" }, true],
  ["git branch --list", { tool_input: { command: "git branch --list" }, tool_name: "Bash" }, false],
  ["git switch to an existing branch", { tool_input: { command: "git switch main" }, tool_name: "Bash" }, false],
  ["branch with an unknown prefix", { tool_input: { command: "npm run branch -- feature/foo" }, tool_name: "Bash" }, true],
  ["branch listing", { tool_input: { command: "npm run branch -- --list" }, tool_name: "Bash" }, false],
  ["commit with an unknown type", { tool_input: { command: 'npm run commit -- --files a.ts --type feature --scope commit --message "x"' }, tool_name: "Bash" }, true],
  ["commit with a malformed scope", { tool_input: { command: 'npm run commit -- --files a.ts --type fix --scope RepoPicker --message "x"' }, tool_name: "Bash" }, true],
  ["commit subject with a trailing period", { tool_input: { command: 'npm run commit -- --files a.ts --type fix --scope commit --message "add the picker."' }, tool_name: "Bash" }, true],
  ["commit subject starting uppercase", { tool_input: { command: 'npm run commit -- --files a.ts --type fix --scope commit --message "Add the picker"' }, tool_name: "Bash" }, true],
  ["commit subject with a conjunction", { tool_input: { command: 'npm run commit -- --files a.ts --type fix --scope commit --message "add the picker and the overview"' }, tool_name: "Bash" }, true],
  ["commit with --message= and a quoted subject", { tool_input: { command: "npm run commit -- --files a.ts --type fix --scope=commit --message='keep the user\\'s quote' --yes" }, tool_name: "Bash" }, false],
